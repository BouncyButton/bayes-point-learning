...
for rule, alpha in model.counter_.most_common(10):
    ...
    print(("\\footnotesize{\\texttt{%s}}" % final_str).replace(" ^ ", " $\\land$ ")
          , end=" \\\\\n")
print('\\hline')
print('\\hline')

print("total & %.3f & " % np.array(
...
print("%.3f & " % (
        (np.array([any([rule.covers(x) for rule, alpha in model.counter_.most_common(10)]) for x in
                   X_train[y_train == 1]]).sum()) /
        (np.array([any([rule.covers(x) for rule, alpha in model.counter_.most_common(10)]) for x in X_train]).sum())),
      end='')
print("%.3f & " % ((np.array(
    [any([rule.covers(x) for rule, alpha in model.counter_.most_common(10)]) for x in X_test[y_test == 1]]).sum()) /
                   (np.array([any([rule.covers(x) for rule, alpha in model.counter_.most_common(10)]) for x in
                              X_test]).sum())), end='')
print("\\\\")
print('\\end{tabular}')
