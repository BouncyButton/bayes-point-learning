...
class BayesianRuleSetClassifier(BayesPointClassifier):
    ...
    def __init__(self,
        ...
        super().__init__(verbose=verbose, bp_verbose=bp_verbose, threshold_acc=threshold_acc, target_class=target_class,
                         pool_size=pool_size,
                         find_best_k=find_best_k, random_state=random_state, encoding=encoding, to_string=to_string,
                         max_rules=max_rules, T=T)
    ...
    def base_method(self, X, y, target_class):
        # TODO does BRS lower its performance if the random state is fixed for all iterations?
...
# taken from imodels repo
# but the original implementation is at
# https://github.com/wangtongada/BOA/blob/master/code/model.py
...
class BRS(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, n_rules: int = 2000,
        ...
        self.alpha_l = alpha_l
        self.beta_l = beta_l
        self.random_state = random_state  # patched?

        self.feature_names_ = None

    def fit(self, X, y, feature_names: list = None, init=None, verbose=False):
        '''
        Parameters
        ----------
    ...
    def _generate_rules(self, X, y, verbose):
        ...
        if self.discretization_method == 'fpgrowth' and self.maxlen <= 3:
            ...
            for length in range(1, self.maxlen + 1, 1):
                n_estimators = min(pow(df.shape[1], length), 2000)
            ...
            unique_rules = list(sorted(set(tuple(x) for x in rules)))
            # shuffle according to seed
            rnd.shuffle(unique_rules)
            rules = [list(x) for x in unique_rules]
        self.rules_ = rules
...
def _extract_rules(tree, feature_names):
    ...
    for child in idx:
        rule = []
        if left.shape == right.shape == (1,) and left.item() == right.item() == -1:
            continue  # patched instances where the tree is trivial (a leaf)
