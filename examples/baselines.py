...
N_SEEDS = 10
T = 1
methods = [AqClassifier]
print(methods)
strategies = ['baseline']  # , 'multi-iteration']
dataset_sizes = [0.5] #, 0.33, 0.25]  # , 0.3333, 0.25, 0.1]  # , 0.3333]  # , 0.1]  # , 0.3333, 0.1]  # [0.5, 0.3]  # , 0.1, 0.03]
POOL_SIZE = 1
            ...
            for dataset_size in dataset_sizes:
                ...
                for seed in range(N_SEEDS):
                    ...
                    if method == AqClassifier:
                        est = method(maxstar=1, verbose=1)
                    else:
                        est = method()
                    ...
                    results.writerow(
                        [name, method.__name__, dataset_size, f1, acc, tn, fp, fn, tp, strategy, T, 1, -1,
                         time.time() - start, seed + use_next_seed, avg_ruleset_len, avg_rule_len])
